...
import 'package:flutterrtdeliveryboyapp/ui/common/facebook_login_web_view.dart';
...
import 'package:flutterrtdeliveryboyapp/viewobject/holder/apple_login_parameter_holder.dart';
import 'package:flutterrtdeliveryboyapp/viewobject/holder/facebook_login_user_holder.dart';
import 'package:flutterrtdeliveryboyapp/viewobject/holder/fb_login_parameter_holder.dart';
import 'package:flutterrtdeliveryboyapp/viewobject/holder/google_login_parameter_holder.dart';
import 'package:flutterrtdeliveryboyapp/viewobject/holder/user_login_parameter_holder.dart';
import 'package:flutterrtdeliveryboyapp/viewobject/holder/user_register_parameter_holder.dart';
...
class UserProvider extends PsProvider {
  UserProvider(
    ...
    subscription = userListStream.stream.listen((PsResource<User> resource) async {
     ...
     await replaceIsDeliveryBoy(_user.data.userFlag);
     await replaceLoginUserId(_user.data.userId);
      ...
      if (!isDispose && resource.status != PsStatus.NOACTION) {
        notifyListeners();
      }
    });
  }
  ...
  /// Apple Login Related
  ///
  Future<void> loginWithAppleId(BuildContext context, String deviceToken,
      Function callback, UserProvider userProvider) async {
    ///
      ...
      if (await Utils.checkInternetConnectivity()) {
        ...
        if (firebaseUser != null) {
          ...
          final PsResource<User> resourceUser =
              await _submitLoginWithAppleId(firebaseUser, deviceToken);
          ...
          if (resourceUser != null && resourceUser.data != null) {
            ...
            /// Success
            ///
           await userProvider.replaceLoginUserData('', '', '', '',
                resourceUser.data.userId, resourceUser.data.userFlag);
  ...
  Future<PsResource<User>> _submitLoginWithAppleId(
      fb_auth.User user, String deviceToken) async {
    if (user != null) {
    ...
    if (isCheckBoxSelect) {
      ...
      if (await Utils.checkInternetConnectivity()) {
        ...
        if (firebaseUser != null) {
          ...
          ///
          final PsResource<User> resourceUser =
              await _submitLoginWithGoogleId(context, firebaseUser);
  ...
  Future<PsResource<User>> _submitLoginWithGoogleId(
      BuildContext context, fb_auth.User user) async {
    if (user != null) {
      String email = user.email;
      if (email == null || email == '') {
        // if (user.providerData.isNotEmpty) {
        ...
        if (facebookLoginUserHolder != null &&
          ...
          final PsResource<User> resourceUser =
              await _submitLoginWithFacebookId(
                  context, facebookLoginUserHolder);
  ...
  Future<void> loginWithEmailId(BuildContext context, String email,
    ...
    if (await Utils.checkInternetConnectivity()) {
      ...
      ///
      final PsResource<User> resourceUser =
          await _submitLoginWithEmailId(context, email, password);
